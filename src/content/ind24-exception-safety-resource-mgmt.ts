...
int main() {
    ...
    callouts: [
      { n: 1, text: 'std::expected<T, E> 讓「預期中會失敗」的路徑成為回傳型別的一部分，不需要丟出例外。' },
      { n: 2, text: 'AggregateReport 同時保留成功與所有失敗的資訊，而非只保留第一個失敗。' },
      { n: 3, text: '啟動固定數量的平行任務，模擬多執行緒各自獨立可能失敗的情境。' },
      { n: 4, text: 'std::launch::async 強制每個任務在獨立執行緒上執行，而非延遲到 get() 才同步執行。' },
      { n: 5, text: 'future::get() 在此只回傳 std::expected 值本身；真正的例外情境改用 exception_ptr 另行處理。' },
      { n: 6, text: '關鍵：任何一個任務失敗都不會中斷聚合迴圈，其餘任務的成功或失敗依然會被完整記錄。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '因為它把失敗編碼進回傳型別，不需要堆疊展開，避免了例外機制在熱路徑上的額外成本' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '讓工作函式定期檢查旗標並提前返回，且中繼狀態本身具強保證（先算完再以單一操作發佈結果）' },
