...
int main() {
    ...
    callouts: [
      ...
      { n: 2, text: '協作式取消的關鍵：定期輪詢旗標，而非被強制中斷；每次迭代都讓執行緒有機會自願離開。' },
      { n: 3, text: 'std::jthread 建構時會建立內部 stop_source，並將對應 stop_token 綁定到函式呼叫。' },
      { n: 4, text: 'std::stop_callback 註冊的回呼會在 request_stop() 呼叫的執行緒上同步執行。' },
      { n: 5, text: 'request_stop() 只是設定旗標，不會強制中斷 worker 正在執行的程式碼。' },
      { n: 6, text: '即使忘記手動 join，jthread 解構子也會自動處理，避免裸 thread 常見的 std::terminate 崩潰。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '透過 native_handle() 取得底層原生控制代碼，交給平台 API（如 pthread_setaffinity_np）設定' },
