...
int main() {
    ...
    callouts: [
      { n: 1, text: '<version> 是取得所有標準功能測試巨集（__cpp_lib_*／__cpp_*）最乾淨的方式，不需要為了巨集特別 include 對應的重量級標頭。' },
      { n: 2, text: '__cpp_lib_jthread 只有在函式庫真的實作了 std::jthread 時才會被定義；語言支援 C++20 不代表函式庫已跟上。' },
      { n: 3, text: 'std::barrier 是 C++20 新增的可重複使用集合點，透過功能測試巨集判斷可用性再決定是否納入編譯單元。' },
      { n: 4, text: '找不到 __cpp_lib_shared_mutex 時退回一般 std::mutex，讀取效能會下降，但至少維持可編譯、可正確運作。' },
      { n: 5, text: '__has_include 是檢查標頭是否存在的可攜寫法，適合用來偵測整個功能區塊（如 <execution>）是否被實作。' },
      { n: 6, text: 'std::jthread 解構時自動 join；若退回 std::thread 則必須顯式呼叫 join，否則物件解構時仍持有可 join 狀態會直接 std::terminate。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '屬於 C++26 討論中的提案／草案階段特性，是否、以何種形式定案仍可能變動，須以官方文件與編譯器發行紀錄為準' },
  ...
  furtherReading: [
    {
      ...
      description: '逐特性列出 GCC／Clang／MSVC 等編譯器支援的標準版本與函式庫落地進度，是查詢當下真實支援狀態的權威來源。',
    },
    {
      title: 'Feature testing (C++20) - cppreference.com',
      href: 'https://en.cppreference.com/w/cpp/feature_test',
      description: '完整的 __cpp_lib_*／__cpp_* 功能測試巨集清單，撰寫可攜式條件編譯程式碼的必查頁面。',
    },
    {
