...
int main() {
    ...
    callouts: [
      { n: 1, text: '刻意讓多個執行緒對同一顆 atomic 變數做高頻率 CAS，用來觀察高競爭下的重試行為。' },
      { n: 2, text: '額外用一個 atomic 計數器統計失敗次數，僅為教學示範；正式程式碼通常不需要。' },
      { n: 3, text: '每次迴圈重新讀取目前值作為 CAS 的期望值。' },
      { n: 4, text: 'compare_exchange_weak 失敗時會自動把 expected 更新為目前實際值，因此可以直接重試。' },
      { n: 5, text: '每一次 CAS 失敗都代表一次讀取與計算被丟棄，這正是 CAS retry storm 的成本來源。' },
      { n: 6, text: '把執行緒數調高並觀察 retries 與 elapsed 的變化，可直接感受到高競爭下無鎖不必然更快。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        { id: 'a', text: 'lock-free 保證每個執行緒都在有限步驟內完成；wait-free 只保證系統整體有進展' },
        { id: 'b', text: 'wait-free 保證每個執行緒都在有限步驟內完成，不受其他執行緒排程影響；lock-free 只保證系統整體至少有一個執行緒能持續前進' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '因為大量執行緒反覆對同一條快取線做 CAS，造成快取一致性流量暴增與持續重試，而鎖在競爭時會讓執行緒睡眠、不產生持續匯流排流量' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '因為其他執行緒可能仍持有指向該節點的指標並正在存取它，直接釋放會造成釋放後使用' },
  ...
  diagram: {
    key: 'generic-flow',
    nodes: ['Wait-free（保證每執行緒有限步驟）', 'Lock-free（保證系統整體前進）', 'Obstruction-free（僅無競爭時保證）', '有鎖備案（睡眠等待，避免匯流排風暴）'],
    caption:
