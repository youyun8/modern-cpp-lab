...
void referenceLookupTable() {
    ...
    callouts: [
      { n: 1, text: 'P2300 涵蓋 sender/receiver 與 std::execution，是全書引用份量最重、修訂也最頻繁的提案，查閱時務必連版本號一起確認。' },
      { n: 2, text: 'P1673 定義 std::linalg，提案文件本身即附帶大量與傳統 BLAS API 對照的範例，可當 API 速查表使用。' },
      { n: 3, text: 'P1928 把 Parallelism TS2 的 std::experimental::simd 扶正為標準 std::simd，兩者語意大致延續但命名空間不同。' },
      { n: 4, text: 'P0019 定義 std::atomic_ref，是本表中最早定案、語意最穩定的一份提案，已隨 C++20 併入標準。' },
      { n: 5, text: '提案編號後常跟著修訂版本（RN），同一編號不同版本的 API 細節可能不同，只寫編號不寫版本容易誤用過時內容。' },
      { n: 6, text: '對不確定的提案編號誠實標註「請查 wg21.link 最新清單」，遠比給出一個看似精確卻可能錯誤的編號更負責任。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: '直接到 wg21.link 加上該提案編號（如 wg21.link/p2300），查看官方文件庫中最新版本的原文' },
        { id: 'c', text: '只要記得提案編號就不需要再查閱，內容不會隨版本變動' },
        { id: 'd', text: '以最早聽說這個提案時的印象為準即可' },
      ],
      correctOptionId: 'b',
      explanation: 'wg21.link 是查閱 WG21 提案原文最直接可靠的入口，加上提案編號即可跳轉到官方文件庫中該提案的最新版本；提案內容常隨審議進度修訂，僅憑印象或部落格轉述容易得到過時或不準確的資訊。',
    },
    {
      id: 'q2',
      ...
      options: [
        ...
        { id: 'b', text: 'cppreference.com 上該功能頁面的「編譯器支援」表格，並對照自己使用的編譯器版本' },
        { id: 'c', text: '任何搜尋引擎排名第一的文章，不需要看發布日期' },
        { id: 'd', text: '猜測所有主流編譯器進度一致，不需要個別查證' },
      ],
      correctOptionId: 'b',
      explanation: 'cppreference 的編譯器支援表格會逐版更新並列出各編譯器（GCC/Clang/MSVC）開始支援某功能的具體版本號，是判斷「此刻能不能用」最直接可靠的依據；提案文件的動機章節說明的是設計理由而非實作進度。',
    },
    {
      id: 'q3',
      ...
      options: [
        ...
        { id: 'b', text: '誠實標註「確切編號請查閱 wg21.link 最新論文清單」，並鼓勵讀者自行查證，而不是給出未經確認的編號' },
        { id: 'c', text: '略過提案編號不談，也不建議任何查證方式' },
        { id: 'd', text: '直接假設它與最相近的另一個提案共用編號' },
      ],
      correctOptionId: 'b',
      explanation: '在不確定精確編號時，誠實引導讀者到 wg21.link 的最新論文索引自行查證，遠比給出一個可能錯誤的編號更負責任；捏造或猜測編號會誤導讀者，且可能因提案拆分、改號而完全失效。',
    },
  ],
  diagram: {
    key: 'generic-flow',
    nodes: ['標準提案', '經典書籍', '線上參考', '延伸閱讀'],
    caption: '四類參考資料互為補充：標準提案定義精確語意與狀態，經典書籍打基礎與補理論，線上參考追蹤最新支援狀況，延伸閱讀連回全書各章節對應主題。',
  },
  tryIt: {
