...
int main() {
  ...
  return 0;
    ...
    callouts: [
      {
        n: 1,
        text: '把 warm-up 次數與正式量測次數分開當參數，強迫呼叫端明確決定兩者，而不是憑感覺跑幾次就當數字。',
      },
      {
        n: 2,
        text: '暖機迴圈的結果完全丟棄：目的只是讓 cache、分支預測器、以及（若未事先鎖定）turbo/DVFS 頻率進入穩定狀態，暖機期間量到的時間不具代表性。',
      },
      {
        n: 3,
        text: '使用 steady_clock（單調時鐘）而非 system_clock，避免系統時間校正（NTP）造成的時間跳動污染量測。',
      },
      {
        n: 4,
        text: '記錄每一次獨立重複的時間到向量中，之後才做統計；絕不只跑一次就回報「這是效能數字」。',
      },
      {
        n: 5,
        text: '常態近似信賴區間假設樣本平均數近似常態分布（中央極限定理），對量測次數足夠多、雜訊非極端偏態時堪用；小樣本或長尾分布（例如受排程器搶佔影響的少數離群值）更適合用 bootstrap 重抽樣估計信賴區間。',
      },
      {
        n: 6,
        text: '中位數對離群值（例如被 OS 排程器搶佔一次而暴增的個別樣本）比平均數更穩健，兩者一起回報能看出分布是否偏態。',
      },
    ],
  },
  deepDive: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '單次執行的時間是效能分布上的一個隨機樣本，可能落在雜訊範圍內；沒有多次重複與信賴區間，無法判斷觀察到的差異是真實差異還是量測雜訊',
        },
    ...
    },
    {
      ...
      options: [
        {
          id: 'a',
          text: '不同節點的 CPU 因製程差異（silicon lottery）能維持的實際 turbo 頻率不同',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '把一組核心從一般排程器的核心集合中移除，使一般行程與中斷不會排到這些核心，讓基準測試獨佔使用，避免被其他行程搶佔造成的延遲尖峰',
        },
  ...
  furtherReading: [
    {
      ...
      description:
        '經典演講，講解為何常見的延遲量測方法系統性低估尾端延遲，以及如何正確思考量測方法論。',
    },
    {
