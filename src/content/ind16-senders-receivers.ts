...
int main() {
    ...
    callouts: [
      {
        n: 1,
        text: 'scheduler 是「排程工廠」，只知道要在哪個執行環境跑，不知道也不需要知道上層的演算法結構。',
      },
      {
        n: 2,
        text: 'schedule(sch) 回傳的 sender 尚未執行任何工作，只是描述「之後要排入 sch 這個環境」。',
      },
      { n: 3, text: 'then 串接接續邏輯，語意等同 future 想要卻做不到的 .then(...)。' },
      {
        n: 4,
        text: 'when_all 讓兩個互不相依的分支各自平行排入執行緒池，等兩者都完成才繼續，取代手動計數器/condvar。',
      },
      {
        n: 5,
        text: 'bulk 對一段索引範圍做資料平行展開，換成 GPU scheduler 時可能被映射成一次 kernel launch。',
      },
      {
        n: 6,
        text: 'sender 是懶惰的：在 sync_wait（或其他 start 觸發點）呼叫之前，整張計算圖完全不會執行。',
      },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: 'sender 是懶惰的描述，必須顯式啟動（如 sync_wait）才會執行；future 建立當下工作通常已經開始執行，且沒有標準化的接續機制',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '因為這樣可以讓同一段演算法結構在不同硬體後端間切換，只需替換 scheduler，讓單一結構化並行圖能橫跨異質後端並被整體最佳化',
        },
  ...
  furtherReading: [
    {
      ...
      description:
        'senders/receivers 與 std::execution 的正式提案文件，涵蓋動機、設計原理與完整 API 規格。',
    },
    {
      title: 'NVIDIA/stdexec — Reference implementation of P2300',
      href: 'https://github.com/NVIDIA/stdexec',
      description:
        'NVIDIA 主導、業界廣泛採用的 P2300 參考實作，可在 C++26 編譯器完整支援前實驗 senders/receivers。',
    },
    {
      title:
        'Working with Asynchrony Generically: A Tour of C++ Executors — Eric Niebler, CppCon 2021',
      href: 'https://www.youtube.com/watch?v=xLboNIf7BTg',
      description:
        'P2300 作者之一 Eric Niebler 講解 sender/receiver 模型與泛用非同步演算法（then／when_all／sync_wait）的設計動機。',
    },
    {
      title: 'std::execution reference — en.cppreference.com',
      href: 'https://en.cppreference.com/w/cpp/execution',
      description:
        'cppreference 上逐步更新的 std::execution API 參考頁面，可追蹤各編譯器實作進度。',
    },
  ],
};
