...
#pragma omp parallel for reduction(+ : local_sum) schedule(static)
    ...
    callouts: [
      { n: 1, text: 'MPI_Init_thread 必須用 required 明確要求執行緒等級；FUNNELED 表示只有主執行緒可以呼叫 MPI，OpenMP worker 執行緒一律不可。' },
      { n: 2, text: '永遠檢查 provided 而非假設 required 被滿足：MPI 實作可能只給予較低等級的支援。' },
      { n: 3, text: '用 std::atomic 取代 OpenMP critical/atomic 子句是漸進遷移的低風險第一步，屬於純 C++ 標準庫層級改動。' },
      { n: 4, text: 'OpenMP 平行迴圈內的熱路徑完全不呼叫 MPI，避免違反 FUNNELED 的限制。' },
      { n: 5, text: '離開 parallel for 之後，程式回到單一執行緒，這裡才是唯一允許呼叫 MPI_Reduce 等 MPI 函式的地方。' },
      { n: 6, text: 'MPI_Reduce 的完成語意來自 MPI 標準自身，不經過 C++ 的 happens-before／atomic 機制，兩者是獨立的同步網域。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: '只有呼叫 MPI_Init_thread 的那個執行緒可以呼叫 MPI 函式，其餘執行緒完全不可呼叫' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '仍需要透過 std::atomic 或 mutex 等標準 C++ 同步原語，明確在兩個執行緒之間建立 happens-before，MPI 的完成語意不會自動延伸到執行緒間' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'c', text: 'std::execution 提供可移植的函式庫層級結構化並行，但在 NUMA 感知排程、schedule 子句與 SIMD 編譯器提示等方面，目前仍不及 OpenMP 成熟；兩者現階段是互補而非取代關係' },
  ...
  diagram: {
    key: 'generic-flow',
    nodes: ['MPI（跨行程通訊）', 'OpenMP（節點內平行迴圈）', 'C++ 標準並行（std::atomic／jthread／std::execution）', '漸進遷移（新程式碼優先、熱路徑最後）'],
    caption:
  ...
  furtherReading: [
    {
      ...
      description: 'MPI 標準文件（MPI Forum 官方頁面），涵蓋 MPI_Init_thread 與四個執行緒支援等級的完整定義。',
    },
    {
      title: 'OpenMP Application Programming Interface Specification',
      href: 'https://www.openmp.org/specifications/',
      description: 'OpenMP 官方規格文件，涵蓋 parallel for 的 schedule／proc_bind 子句與 SIMD 指示詞。',
    },
    {
    ...
    },
    {
      ...
      description: 'MPI Forum 對 hybrid MPI + threads 常見問題的說明，包含各實作對 MPI_THREAD_MULTIPLE 支援程度的實務落差。',
    },
  ],
};
