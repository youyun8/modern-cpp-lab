...
Task<int> overlap_comm_and_compute(int n) {
  ...
  co_return partial * 2;
    ...
    callouts: [
      {
        n: 1,
        text: 'Task<T> 是可被 co_await 的協程回傳型別，內部以 coroutine_handle 串接完成後的延續。',
      },
      { n: 2, text: 'continuation 記錄「這個 Task 完成後該恢復哪個協程」，取代手寫回呼。' },
      {
        n: 3,
        text: 'FinalAwaiter::await_suspend 回傳 handle 而非呼叫 resume()，即為對稱轉移的關鍵寫法。',
      },
      {
        n: 4,
        text: '沒有 continuation（最外層協程）時回傳 std::noop_coroutine()，代表「不轉移，直接返回排程器」。',
      },
      {
        n: 5,
        text: 'Task 自身的 await_suspend 也回傳 handle，讓 co_await task 同樣走保證尾呼叫路徑。',
      },
      {
        n: 6,
        text: '此處以同步運算模擬非同步通訊；實際 HPC 場景會把 handle 交給 MPI 或裝置的完成回呼。',
      },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '因為每次 resume() 呼叫都可能在呼叫堆疊上疊加一層框架，長鏈情境下堆疊會無界成長',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '讓 await_suspend 回傳欲恢復的 std::coroutine_handle，使編譯器以保證尾呼叫方式切換協程',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '它消除手寫 promise_type／handle 管理的需要，並支援配置器與遞迴 yield（elements_of）',
        },
