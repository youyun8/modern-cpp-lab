...
const ind02HardwareReality: ChapterContent = {
  ...
  concept: {
    standard: 'C++20',
    body:
      '所有平行程式最終都跑在真實硬體上：多顆核心透過快取一致性協定（如 MESI）共享記憶體視圖，一致性流量會隨核心數成長而成為 scaling 天花板。NUMA 系統中記憶體有遠近之分，跨節點存取延遲可達數倍。硬體為求效能會亂序執行並使用 store buffer，因此各架構提供的「記憶體模型」強弱不同——x86 是較強的 TSO，ARM／Power 是弱序模型，需要顯式屏障才能保證跨執行緒可見順序。理解這些現實，才能寫出既正確又能真正 scale 的 lock-free 與平行程式碼。',
  },
  code: {
    lang: 'cpp',
    code: `#include <atomic>
...
double benchmarkIncrements(Counters& counters, std::size_t iterations) {
    ...
    callouts: [
      ...
      { n: 5, text: '使用 memory_order_relaxed 是刻意的：這裡只在意計數正確與快取行為，不需要額外的順序保證。' },
      { n: 6, text: '量測方法：分別對 FalseSharingCounters 與 PaddedCounters 執行本函式並比較耗時，即可觀察 false sharing 造成的 scaling 崩潰。' },
    ],
  },
  deepDive: [
    {
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: 'x86 的 TSO 記憶體模型較強，掩蓋了程式碼中缺少的同步語意；ARM 屬弱序模型，允許更多重排' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'c', text: '將程序的 CPU 執行與記憶體配置都綁定在同一個 NUMA 節點，避免跨節點存取延遲' },
