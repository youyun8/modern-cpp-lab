...
double parallel_sum(NumaBuffer& buf, unsigned num_threads) {
    ...
    callouts: [
      {
        n: 1,
        text: '若無 alignas(64)，相鄰的每執行緒累加器仍可能落在同一快取行；跨插槽（socket）的 false sharing 比單插槽內更昂貴，因為它會觸發跨節點的一致性流量。',
      },
      {
        n: 2,
        text: 'NumaBuffer 只是普通的 std::vector；配置時（建構子）尚未產生實體頁面，真正決定落點的是之後的第一次寫入。',
      },
      {
        n: 3,
        text: 'first-touch 政策：實體記憶體在第一次被寫入時才配置，且配置在執行該寫入的執行緒目前所在的 NUMA 節點——這是本章最核心的機制。',
      },
      {
        n: 4,
        text: '每個執行緒只初始化自己負責的區段，讓資料均勻分散到各 NUMA 節點，而非全部集中在呼叫 new 的那顆核心所在節點。',
      },
      {
        n: 5,
        text: '運算階段刻意重用與初始化階段相同的切塊方式，確保每個執行緒之後存取的仍是自己（或同節點）first-touch 過的記憶體，維持區域性。',
      },
      {
        n: 6,
        text: 'partial[t] 各自佔滿一條快取行，多執行緒同時寫入不同插槽（slot）不會互相使快取失效。',
      },
    ],
  },
  deepDive: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        {
          id: 'c',
          text: '該記憶體第一次被寫入（touch）的當下，配置在執行該次寫入的執行緒所在節點',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '因為 first-touch 政策讓整塊資料全落在初始化執行緒所在的單一節點，其餘節點的執行緒之後都要付出遠端記憶體存取的延遲與頻寬代價',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'c',
          text: '多插槽下的一致性流量需經過插槽間互連，延遲與代價通常高於同插槽內的核心對核心一致性流量',
        },
...
int main() {
  ...
  furtherReading: [
    ...
    },
    {
      ...
      description:
        'numactl 指令的用法，包含 --cpunodebind、--membind、--interleave 等節點綁定選項。',
    },
    {
