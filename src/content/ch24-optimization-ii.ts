...
long long sumAboveThreshold(const std::vector<int>& v, int t) {
  ...
  return s;
    ...
    callouts: [
      {
        n: 1,
        text: '兩個相鄰的計數器常落在同一快取行；不同核心寫入會互相使對方快取失效，即偽共享。',
      },
      {
        n: 2,
        text: 'alignas(hardware_destructive_interference_size) 讓物件對齊到快取行邊界，避免共享。',
      },
  ...
  deepDive: [
    {
      ...
      body: '陣列的結構（AoS，`struct{float x,y,z;} a[N]`）直觀但在只用單一欄位時浪費快取頻寬；結構的陣列（SoA，`float x[N], y[N], z[N]`）讓同欄位連續存放，對向量化與快取極友善，是熱路徑常見的重構。\n\n對齊（`alignas`）讓 SIMD 載入不跨快取行；適度的填充可消除偽共享。佈局決策應由熱路徑的存取模式驅動。',
    },
    {
      heading: '分支的成本與消除',
      body: '難以預測的分支會頻繁清空管線。可用無分支技巧（條件移動、查表、算術遮罩）取代，或先排序資料讓分支變得可預測。`[[likely]]`／`[[unlikely]]` 或 `__builtin_expect` 可給提示。\n\n但無分支不一定更快，且會損害可讀性——務必以量測佐證，只在證實為瓶頸的熱路徑上採用。',
    },
    {
      heading: 'Prefetch 與迴圈最佳化',
      body: '軟體預取（`__builtin_prefetch`）偶爾有用，但過度預取會污染快取、反而更慢；多數情況應信賴硬體預取器並改善存取的規律性。迴圈展開（unrolling）與迴圈不變量外提可減少額外負擔並釋放 ILP。\n\n分塊（blocking／tiling）把工作切成能放進快取的區塊以最大化重用，是矩陣運算等記憶體受限核心的關鍵手法。',
    },
  ],
  pitfalls: [
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '因為快取以快取行載入，連續存取能重複利用已載入資料，減少昂貴的 DRAM 存取',
        },
