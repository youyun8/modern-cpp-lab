...
int main() {
  ...
  return 0;
    ...
    callouts: [
      ...
      {
        n: 2,
        text: 'lower_bound 在已排序區間以二分搜尋找第一個不小於目標的元素，複雜度 O(log n)。',
      },
  ...
  deepDive: [
    {
      ...
      body: '`std::vector` 是預設；已知大小時 `reserve` 可避免多次重新配置與元素移動。節點型容器（`list`、`map`）雖有 O(1)／O(log n) 的插入保證，但每元素獨立配置、快取不友善，實測常慢於 `vector`。\n\nC++23 的 `std::flat_map`／`flat_set` 以排序 `vector` 實作，查找為 O(log n) 但快取友善、迭代快，適合讀多寫少的場景。選容器要同時看漸進複雜度與實際記憶體行為。',
    },
    {
      heading: '迭代器失效與 ranges 投影',
      body: '各容器有明確的迭代器／參考失效規則：`vector` 的 `push_back` 觸發重新配置會使所有迭代器失效；`unordered_map` 的元素參考在 rehash 後仍穩定（但迭代器失效），而 `vector` 的參考不穩定。誤解這些規則是常見的懸置來源。\n\nranges 演算法接受投影（projection），如 `std::ranges::sort(v, {}, &Item::key)` 直接依成員排序，並可用 `|` 組合視圖，寫出宣告式且不易錯的資料處理。',
    },
    {
      heading: '演算法與慣用法',
      body: '`std::sort` 為 introsort（quicksort + heapsort 保底 O(n log n)），需穩定性時用 `stable_sort`。插入建構物件用 `emplace_back` 就地建構，避免多餘的暫時物件與移動。\n\n移除元素用 erase-remove 慣用法，或 C++20 更直觀的 `std::erase`／`std::erase_if`。理解這些慣用法能寫出正確且高效的容器操作。',
    },
  ],
  pitfalls: [
  ...
  diagram: {
    ...
    caption: 'STL 的組成：容器儲存資料，迭代器抽象走訪，ranges 與演算法在其上提供可組合的操作。',
  },
  tryIt: {
    code: `#include <algorithm>
#include <iostream>
