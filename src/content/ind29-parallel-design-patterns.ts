...
const ind29ParallelDesignPatterns: ChapterContent = {
  ...
  concept: {
    standard: 'C++20',
    body:
      '並行設計樣式是把「多個執行流如何協作」的重複性問題結晶成可重用的結構。Producer-Consumer 以有界佇列解耦生產速率與消費速率；Pipeline 把工作切成多個階段，每個階段各自佔用一條或多條執行緒，資料像水流一樣依序流過；Fork-Join 是遞迴分治：把問題「fork」成子任務平行執行，再於「join」點彙整結果；Actor 讓每個運算單元只透過訊息傳遞溝通、彼此不共享可變狀態；Active Object 把方法呼叫（介面）與方法執行（實作、排程）解耦，呼叫端把請求丟進佇列即可返回，實際執行由專屬執行緒處理；Monitor 則是把同步邏輯封裝進物件本身，對外只曝露執行緒安全的操作，呼叫者無需關心鎖的存在。這些樣式並非互斥，實務上常組合使用，例如以 Active Object 包裝一個內部使用 Producer-Consumer 佇列的服務。',
  },
  code: {
    lang: 'cpp',
...
int main() {
    ...
    callouts: [
      ...
      { n: 2, text: '呼叫端得到 std::future，可選擇立即返回或稍後 wait／get 結果，達成非同步語意。' },
      { n: 3, text: '請求被打包成任務放入內部佇列，等同 Producer-Consumer 中生產者的角色。' },
      { n: 4, text: '所有共享狀態（queue_、stop_）只在 mutex_ 保護下存取，符合 Monitor 樣式的精神。' },
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: 'Actor 強調狀態完全私有、只透過訊息傳遞溝通；Monitor 則是把鎖封裝進物件的方法中，仍允許以同步方法呼叫存取共享狀態' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '因為每條執行緒需要獨立堆疊記憶體，且執行緒數一多，排程器的 context-switch 與快取失效成本會急遽上升，往往在 CPU 算力被用滿之前就先耗盡記憶體或排程頻寬' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: 'BSP（Bulk Synchronous Parallel）：各 worker 完成本地計算後同步等待，全部到齊才進入下一個超步' },
...
int main() {
  ...
  furtherReading: [
    ...
    },
    {
      ...
      description: '收錄 Active Object、Monitor Object、Reactor、Half-Sync/Half-Async 等並行樣式的經典參考書資訊頁。',
    },
    {
      title: 'The Reactor Pattern',
      href: 'https://www.dre.vanderbilt.edu/~schmidt/PDF/reactor-siemens.pdf',
      description: 'Douglas Schmidt 對 Reactor（事件迴圈）樣式的原始論文，說明如何以單執行緒多工處理多個 I/O 事件來源。',
    },
    {
      title: 'C++ Concurrency in Action — 樣式相關章節資訊',
      href: 'https://www.manning.com/books/c-plus-plus-concurrency-in-action-second-edition',
      description: 'Anthony Williams 著作，涵蓋以 future／task 為基礎的 fork-join 與訊息傳遞式並行設計實務。',
    },
  ],
};
