...
void scale_tiled(float* __restrict data, std::size_t n, float factor) {
    ...
    callouts: [
      { n: 1, text: '__restrict 是 GCC/Clang 對 C99 restrict 的 C++ 擴充：承諾 y、x 不重疊，消除別名分析的最壞情況假設。' },
      { n: 2, text: 'const 讓編譯器知道 x 不會被此函式修改，進一步簡化別名與資料流分析。' },
      { n: 3, text: '迴圈本體無分支、逐一連續存取 y[i]/x[i]，是自動向量化器最容易處理的形態。' },
      { n: 4, text: 'alignas(32) 讓緩衝區對齊 AVX2 的 256 位元向量寬度，避免未對齊存取的額外開銷。' },
      { n: 5, text: 'kTile 大小的區塊讓資料重複使用發生在快取仍保有該資料時，是 tiling 改善記憶體頻寬瓶頸的核心手法。' },
      { n: 6, text: '__builtin_prefetch 提示硬體提前載入下一區塊，隱藏跨區塊邊界的記憶體延遲；濫用會污染快取，需以量測驗證。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: '因為 C++ 一般指標未被標準假設為不重疊，編譯器必須保守考慮最壞情況下的別名，除非有 __restrict 等額外資訊排除它' },
        { id: 'c', text: '因為 float 型別本身不支援 SIMD 指令' },
        { id: 'd', text: '因為向量化只對整數陣列有效' },
      ],
      correctOptionId: 'b',
      explanation: 'C/C++ 標準不像 Fortran 那樣預設參數不重疊；編譯器必須假設最壞情況（別名存在），因此需要 __restrict 或執行期別名檢查才能安全向量化。',
    },
    {
      id: 'q2',
      ...
      options: [
        ...
        { id: 'b', text: '因為 SIMD load/store 指令偏好連續記憶體，跨步存取需要較慢的 gather/scatter 指令，編譯器的成本模型可能因此判定向量化不划算' },
        { id: 'c', text: '因為跨步存取一定會導致編譯錯誤' },
        { id: 'd', text: '因為只有分支才會阻止向量化，其餘情況必定成功' },
      ],
      correctOptionId: 'b',
      explanation: '向量化的收益取決於能否用高效的連續 load/store；跨步存取通常只能靠昂貴的 gather/scatter 完成，成本模型可能因此認定向量化不值得，即使邏輯上可行。',
    },
    {
      id: 'q3',
      ...
      options: [
        ...
        { id: 'b', text: '回報哪些迴圈未能被向量化以及失敗的具體原因，讓工程師針對性地修改程式碼（如加 __restrict 或改善存取模式）' },
        { id: 'c', text: '關閉編譯器的最佳化以加快編譯速度' },
        { id: 'd', text: '產生組合語言輸出取代原始碼' },
      ],
      correctOptionId: 'b',
      explanation: '這兩個診斷旗標會列出未成功向量化的迴圈與原因（例如別名疑慮、非連續存取、成本模型判定不划算），是診斷與修正向量化障礙的關鍵工具，而非自動修復或關閉最佳化。',
    },
  ],
  diagram: {
    key: 'cache-line',
...
int main() {
  ...
  furtherReading: [
    ...
    },
    {
      ...
      description: 'Clang 官方文件，說明 -Rpass / -Rpass-missed / -Rpass-analysis 最佳化報告旗標的用法。',
    },
    {
