...
int main() {
    ...
    callouts: [
      ...
      { n: 3, text: 'std::make_unique 於 heap 配置並以 unique_ptr 擁有，離開作用域自動 delete，避免洩漏。' },
      { n: 4, text: 'const 宣告不可變狀態，是常數正確性的基礎，也讓編譯器與讀者都能信任該值不變。' },
  ...
  deepDive: [
    {
      ...
      body:
        '自動、動態、靜態與執行緒儲存期各有生命週期規則。跨轉譯單元的非區域靜態物件初始化順序未定義，即著名的 static initialization order fiasco；以「函式內 static 區域變數」（Meyers singleton）或 `constinit` 可規避。\n\n回傳指向區域變數的參考／指標會產生懸置；被移動後的物件處於有效但未指定狀態。清楚掌握誰擁有、誰只是借用，是避免記憶體錯誤的核心。',
    },
    {
      heading: 'const 家族：const／constexpr／consteval／constinit',
      body:
        '`const` 表達不可修改的常數正確性；`constexpr` 要求可在編譯期求值（但也可在執行期）；`consteval`（C++20）強制立即函式必於編譯期求值；`constinit` 保證靜態儲存期物件於編譯期完成初始化，避免 SIOF 又不強制 const。\n\n善用它們能把更多錯誤移到編譯期，並讓常數進入樣板參數、陣列大小等常數運算式脈絡。',
    },
    {
      heading: '轉換家族與 RTTI 成本',
      body:
        '`static_cast` 做編譯期已知的安全轉換；`dynamic_cast` 在多型階層做執行期檢查，依賴 RTTI 且有成本；`const_cast` 僅增刪 cv 限定，用它去修改本質為 const 的物件是 UB；`reinterpret_cast` 位元重解讀，違反嚴格別名時是 UB。\n\n型別雙關應改用 `std::bit_cast`（C++20）。避免 C 風格轉換，因為它會在上述語意間靜默切換，難以審查。',
    },
  ],
  pitfalls: [
