...
int main() {
    ...
    callouts: [
      ...
      {
        n: 3,
        text: 'std::atomic_ref 是包裹既有記憶體的「原子視圖」，不需把整個陣列宣告成 atomic 型別。',
      },
      {
        n: 4,
        text: 'fetch_add 用 memory_order_relaxed：每個 bucket 只是獨立計數，不需要與其他資料排序。',
      },
      { n: 5, text: '陣列本身仍是普通、可平凡複製的記憶體，可與 SIMD／既有函式庫互通。' },
      {
        n: 6,
        text: 'join 建立了執行緒之間的同步點，之後主執行緒讀取 histogram 保證看到所有累加結果。',
      },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'c',
          text: 'atomic_ref 讓陣列保持一般記憶體佈局，與既有非原子程式碼（如 SIMD、BLAS）相容，只在需要時對個別元素做原子存取',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '節點 A 被彈出並釋放後，另一次配置又剛好在相同位址建立新節點，使 CAS 誤判「值沒變」而寫入已失效的內部狀態',
        },
