...
BENCHMARK_MAIN();`,
    callouts: [
      {
        n: 1,
        text: '待測函式本身有明確的資料相依鏈（累加），本身不易被消除，但呼叫端若丟棄回傳值仍可能被整段刪除。',
      },
      {
        n: 2,
        text: '這是常見錯誤示範：回傳值未使用時，最佳化器可以合法地把整個迴圈與函式呼叫都視為無副作用而刪除。',
      },
      {
        n: 3,
        text: '把回傳值存起來，並用 DoNotOptimize 標記，讓編譯器認為之後還會用到這個值，不能消去計算。',
      },
      {
        n: 4,
        text: 'DoNotOptimize 的實作通常是把值綁到一個內嵌組語的假讀取，強迫其真正被計算並保留在暫存器或記憶體中。',
      },
      {
        n: 5,
        text: '對於「透過寫入陣列產生副作用」而非「回傳單一值」的函式，DoNotOptimize 不夠，需要另一種手段涵蓋整塊記憶體。',
      },
      {
        n: 6,
        text: 'ClobberMemory 插入一個編譯器記憶體屏障，強迫所有先前的寫入真的落地，避免被延後或與計時窗口重疊。',
      },
    ],
  },
  deepDive: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '編譯器可能合法地把整段計算視為無副作用而刪除，導致量到接近零的空迴圈時間',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: 'DoNotOptimize 固定單一回傳值不被消除，ClobberMemory 強迫先前的記憶體寫入真正落地，兩者常搭配用於不同型態的副作用',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '核心執行期間完成的總浮點運算數，除以核心搬運的總位元組數（可用 LIKWID 等硬體計數器工具量測）',
        },
  ...
  furtherReading: [
    {
      ...
      description:
        '官方原始碼與 User Guide，涵蓋 DoNotOptimize、ClobberMemory 與統計重複的完整用法。',
    },
    {
    ...
    },
    {
      ...
      description:
        '輕量硬體計數器工具集，含 likwid-perfctr 的 FLOPS／MEM 效能群組，適合手動驗證 Roofline 座標。',
    },
    {
