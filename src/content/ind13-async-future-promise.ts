...
const ind13AsyncFuturePromise: ChapterContent = {
  ...
  concept: {
    standard: 'C++11',
    body: 'std::promise／std::future 是一組「一次性、單向」的資料通道：一個 promise 由生產端設定值（或例外），對應的 future 由消費端呼叫 get() 取得，且 get() 只能成功呼叫一次。std::packaged_task 把任意可呼叫物件包裝成「呼叫時自動填好對應 future」的物件，讓你不必手動操作 promise。std::async 則進一步把「建立 packaged_task、丟到執行緒（或延後執行）、回傳 future」三件事包成一個函式呼叫。這一整組設施從 C++11 引入，語意到 C++20 都沒有實質改變：future 本身仍然「不可組合」——沒有 .then()、沒有延續（continuation），想在一個非同步工作完成後接著做下一件事，只能阻塞呼叫 get() 再手動發起下一個 async。這正是後續章節要介紹的 P2300 std::execution（senders/receivers）想解決的核心痛點。',
  },
  deepDive: [
    {
      heading: 'promise／future 的基本契約',
      body: '`std::promise<T>` 與 `std::future<T>` 是一對共享同一塊「共享狀態（shared state）」的物件：生產端持有 promise，呼叫 `set_value` 或 `set_exception` 恰好一次；消費端持有對應的 future，呼叫 `get()` 取回結果，若生產端設的是例外，`get()` 會在消費端重新丟出。這是「單一生產者、單一消費者、一次性（single-shot）」的契約：`future::get()` 只能呼叫一次，第二次呼叫是未定義前提下的合法丟例外（`std::future_error`，錯誤碼 `no_state`），因為呼叫後共享狀態即被消耗。如果需要多個消費端各自讀取同一結果，必須用 `future::share()` 轉成 `std::shared_future`，它允許多次、多執行緒呼叫 `get()`。',
    },
    {
      heading: 'packaged_task：把可呼叫物件包成 future 來源',
      body: '手動操作 promise 容易出錯（忘記在例外路徑上呼叫 `set_exception`、忘記涵蓋所有 return 路徑）。`std::packaged_task<R(Args...)>` 包裝一個可呼叫物件，呼叫 `task.get_future()` 取得對應 future，之後每次「呼叫」這個 task（像呼叫函式一樣）就會執行包裝的可呼叫物件，並自動把回傳值或例外寫進共享狀態。這讓你可以把 task 丟給 `std::thread`、丟進執行緒池的工作佇列，或直接同步呼叫，而不用自己管理 promise 的生命週期與例外轉發。packaged_task 本身「只能移動、不能複製」，這也提醒它代表的是一次性的執行單元。',
    },
    {
...
int main() {
    ...
    callouts: [
      ...
      {
        n: 2,
        text: '顯式指定 std::launch::async，避免預設策略可能選擇 deferred 而在 get() 時才同步執行。',
      },
      {
        n: 3,
        text: 'future 沒有 .then()：串接下一步只能阻塞目前執行緒等待 get() 回傳，無法用回呼延續。',
      },
      {
        n: 4,
        text: '例外只在呼叫 get() 的那一刻才浮現；若從不呼叫 get()，例外會在 future 解構時被靜默吞掉。',
      },
      {
        n: 5,
        text: 'senders/receivers（P2300）把成功值與錯誤都視為 pipeline 的一部分，不必集中在同步點處理。',
      },
      {
        n: 6,
        text: 'future::get() 是一次性操作：共享狀態被消耗後，第二次呼叫會丟出 std::future_error。',
      },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        {
          id: 'c',
          text: '實作可自行選擇 async 或 deferred，deferred 時 func 要到呼叫 get()／wait() 才會同步執行',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '因為 future 的介面只有阻塞式的 get()/wait()，沒有讓「完成後自動接著執行下一步」的機制，必須阻塞一個執行緒才能串接後續工作',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'c',
          text: '第二次呼叫會丟出 std::future_error，因為共享狀態在第一次 get() 後已被消耗',
        },
