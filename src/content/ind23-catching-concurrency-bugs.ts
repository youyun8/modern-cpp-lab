...
//   shared_counter.fetch_add(1, std::memory_order_relaxed);  // [6]`,
    callouts: [
      ...
      { n: 2, text: '`++shared_counter` 展開後是讀取、加一、寫回三個步驟，兩個執行緒交錯執行時可能互相蓋掉對方的結果，是典型的資料競爭。' },
      { n: 3, text: 'std::jthread 解構時自動 join，避免忘記 join 造成的 std::terminate，但不會消除執行緒間的資料競爭。' },
      { n: 4, text: 'TSan 的偵測是動態的：它只能回報這次執行中實際交錯到的競爭，排程序列化時同一份程式可能不會觸發任何警告。' },
      { n: 5, text: '在有競爭的情況下，最終印出的 counter 值通常小於 200000，且每次執行結果可能不同——這正是並行 bug 難以用單次觀察判斷正確性的原因。' },
      { n: 6, text: '改用 std::atomic 的 fetch_add 讓遞增成為一個不可分割的原子操作，消除了讀-改-寫之間被其他執行緒插入的可能，relaxed 已足夠因為這裡沒有其他資料需要被這次遞增排序。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: 'TSan 是動態分析工具，只能回報這次執行實際交錯到的競爭；沒被排程器選中的交錯即使有競爭也不會被觀察到' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '兩者通常不會編譯進同一個二進位，因插樁機制衝突與效能開銷疊加，實務上分開建置、分開執行' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '讓一次偶然觀察到的競爭或崩潰可以被完整記錄並精確重播，不必依賴重新執行時排程恰好再次命中同一個交錯' },
...
int main() {
  ...
  furtherReading: [
    ...
    },
    {
      ...
      description: '線上 litmus test 模擬器，可窮舉列出記憶體序組合在 C++ 記憶體模型下所有允許的結果，與本章經驗性工具互補。',
    },
  ],
};
