...
const ind21ThreadPoolsScheduling: ChapterContent = {
  ...
  title: '執行緒池與任務排程',
  group: 'N · 第六部：效能工程',
  description: 'work-stealing 排程、任務粒度與負載平衡、優先權反轉，以及自建 vs TBB／OpenMP runtime 的取捨。',
  concept: {
    standard: 'C++20',
    body:
...
public:
    ...
    callouts: [
      { n: 1, text: '所有 worker 共享同一個 std::queue，正確且易懂，但代表每次 push/pop 都要搶同一把 mutex_。' },
      { n: 2, text: '執行緒在建構子啟動時就常駐、不斷從佇列取任務，避免每個任務都要付出建立執行緒的成本。' },
      { n: 3, text: '解構子設定 stopping_ 並喚醒所有等待中的 worker，讓它們檢查旗標後正常結束迴圈。' },
      { n: 4, text: 'Submit 只喚醒一個等待中的 worker（notify_one），因為只新增了一筆任務，不需要驚群喚醒。' },
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: '本地從頭部取（LIFO，延續快取局部性），偷竊從尾部拿（FIFO）；分散在不同端點可降低爭用，且尾部任務通常較粗' },
        { id: 'c', text: '本地從尾部取，偷竊從頭部拿；純粹是實作慣例，沒有效能考量' },
        { id: 'd', text: '一律隨機挑選端點，藉此避免任何規律導致的爭用' },
      ],
      correctOptionId: 'b',
      explanation: '本地端優先存取剛產生、與快取局部性相關的任務（頭部，LIFO），空閒執行緒偷竊時改拿另一端（尾部，FIFO），讓兩種操作大多落在不同位置以降低爭用，且尾部任務通常涵蓋較大工作量，減少重複竊取次數。',
    },
    {
      id: 'q2',
      stem: '任務粒度切得「太細」與「太粗」分別會造成什麼問題？',
      options: [
        { id: 'a', text: '太細會排程開銷主導整體時間；太粗容易造成負載不平衡（有些執行緒早完成、有些還在忙且無細任務可偷）' },
        { id: 'b', text: '太細會造成記憶體洩漏；太粗會造成死結' },
        { id: 'c', text: '兩者都只影響編譯時間，不影響執行期效能' },
        { id: 'd', text: '太細會提升快取命中率；太粗會降低快取命中率，兩者都與負載平衡無關' },
      ],
      correctOptionId: 'a',
      explanation: '任務太細時，建立任務、推入/彈出佇列等排程開銷會超過任務本身的計算量；任務太粗時，若各任務耗時不均，提早完成的執行緒缺乏足夠細的任務可偷，導致總時間被最慢任務拖累。',
    },
    {
      id: 'q3',
      stem: '為什麼「執行緒池依優先權派送任務」無法單獨防止優先權反轉？修正這個問題的經典機制是什麼？',
      options: [
        { id: 'a', text: '因為任務仍可能共享以一般 mutex 保護的資源，低優先權任務持有鎖時仍會間接卡住高優先權任務；經典修正是優先權繼承（priority inheritance）' },
        { id: 'b', text: '因為排程器的優先權比較永遠是錯的；修正方式是完全禁止任務共享任何資源' },
        { id: 'c', text: '這種情況不會發生，只要排程器依優先權派送任務就一定不會反轉' },
        { id: 'd', text: '因為 condition_variable 本身有 bug；修正方式是改用忙等（busy-wait）取代阻塞等待' },
      ],
      correctOptionId: 'a',
      explanation: '排程層級的優先權只決定誰先被派送執行，並不能防止鎖層級的阻塞：低優先權任務持有共享鎖時，高優先權任務仍要排隊等鎖。經典修正是優先權繼承，暫時把等待者的優先權借給鎖持有者，讓它盡快釋放鎖。',
    },
  ],
  diagram: {
    key: 'thread-timeline',
    caption: '多個工作執行緒各自的任務時間軸：本地佇列頭部優先執行、空閒執行緒向他人佇列尾端偷取任務，形成動態負載平衡。',
  },
  tryIt: {
    code: `#include <atomic>
  ...
  furtherReading: [
    {
      ...
      description: 'Intel oneTBB（原 TBB）task_arena 官方文件，說明如何控制 work-stealing 排程器的並行度與工作隔離。',
    },
    {
    ...
    },
    {
      ...
      description: 'Blumofe 與 Leiserson 提出 work-stealing 排程理論分析的經典論文，Cilk runtime 的理論基礎。',
    },
    {
      title: 'Priority Inversion — Wikipedia',
      href: 'https://en.wikipedia.org/wiki/Priority_inversion',
      description: '優先權反轉的定義、Mars Pathfinder 案例，以及優先權繼承（priority inheritance）等修正機制概述。',
    },
  ],
};
