...
// [4] Roofline：算術強度 AI = FLOPs / Bytes；可達效能 = min(peak_flops, AI * peak_bw)。
...
int main() {
    ...
    callouts: [
      { n: 1, text: 'Amdahl 定律假設問題規模固定，只探討「用更多核心能把既有工作做多快」，也就是 strong scaling。' },
      { n: 2, text: '當 s 趨近無限大時，speedup 收斂到 1 / (1 - p)：序列部分決定了加速比的硬上限。' },
      { n: 3, text: 'Gustafson 定律換一個角度：核心數增加時，讓問題規模一起變大，凸顯的是 weak scaling 下的可擴充性。' },
      { n: 4, text: 'Roofline 模型把效能上限畫成兩條線的交會：低 AI 時受限於記憶體頻寬，高 AI 時受限於算力峰值。' },
      { n: 5, text: '這一行就是 Roofline 的核心公式：可達效能是「算力峰值」與「AI 乘以頻寬峰值」兩者中較小的一個。' },
  ...
  furtherReading: [
    {
      ...
      description: 'Roofline 模型的原始論文，定義 arithmetic intensity 與可達效能的公式與圖示方法。',
    },
    {
      title: 'STREAM: Sustainable Memory Bandwidth Benchmark (John D. McCalpin)',
      href: 'https://www.cs.virginia.edu/stream/',
      description: 'McCalpin 的 STREAM benchmark 官方頁面，Copy/Scale/Add/Triad 的定義與各平台實測數據。',
    },
    {
