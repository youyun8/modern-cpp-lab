...
const appendixCUbAntipatterns: ChapterContent = {
  ...
  concept: {
    standard: 'C++20',
    body:
      '本附錄彙整全書出現過的並行反面教材（anti-pattern），每一則都以「錯誤寫法」對照「修正寫法」呈現，方便快速查閱與 code review 時比對。涵蓋範圍包括：未同步的共享變數讀寫（資料競爭）、誤用 `volatile` 當作跨執行緒同步原語、無鎖程式碼中的 ABA 問題、無鎖結構忘記延後記憶體回收導致 use-after-free、鎖排序不一致造成的死結、相鄰原子變數未做 padding 造成的 false sharing，以及在 `par_unseq` 演算法中使用不安全（內部上鎖或會拋例外）的可呼叫物件。這些反面教材大多不會在每次執行都重現，測試通過不代表正確；真正的判準是程式碼是否符合 C++ 記憶體模型與各演算法執行策略的前提要求。',
  },
  code: {
    lang: 'cpp',
    code: `#include <atomic>
...
int main() {
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: 'False sharing：兩個計數器落在同一條 cache line，跨核心寫入互相使對方快取失效' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '在 par_unseq 中使用不安全的可呼叫物件：向量化交錯執行下拋出例外是未定義行為' },
  ...
  diagram: {
    key: 'generic-flow',
    nodes: ['資料競爭 / volatile 誤用', 'ABA 問題', '無鎖回收 use-after-free', '死結（鎖排序不一致）', 'False sharing', 'par_unseq 不安全可呼叫物件'],
    caption:
