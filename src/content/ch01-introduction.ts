...
const ch01Introduction: ChapterContent = {
  ...
  concept: {
    standard: 'C++23',
    body: 'C++ 由 Bjarne Stroustrup 於 1980 年代以「C with Classes」為起點發展而來，目標是在保留 C 的執行效率與底層控制的同時，加入抽象化能力。其核心設計哲學可概括為三點：零成本抽象（zero-overhead abstraction，你不使用的特性不需付出代價，你使用的特性也無法再手寫得更快）、直接對應硬體模型，以及以型別系統與 RAII 靜態保證資源正確性。自 C++11 起改為三年一次的標準週期（C++11／14／17／20／23），語言持續現代化。今日 C++ 廣泛用於遊戲引擎、瀏覽器、資料庫、高頻交易、嵌入式系統，以及機器學習與 HPC 的核心運算層。',
  },
  code: {
    lang: 'cpp',
    code: `#include <print>  // C++23: type-safe, formatted output [1]
#include <string>
#include <vector>

// A "modern" Hello World: leverage the standard library's high-level
// abstractions instead of manually managing memory or using C-style I/O.
int main() {
    std::vector<std::string> langs{"C", "C++98", "C++11", "Modern C++"};  // [2]

    for (const auto& lang : langs) {           // [3] range-based for + auto
        std::println("Hello from {}!", lang);  // [4] std::format style
    }
    return 0;  // [5] returning 0 means success
}`,
    callouts: [
      {
        n: 1,
        text: 'C++23 的 <print> 提供 std::print／std::println，取代 iostream 的冗長語法，同時保有型別安全。',
      },
      {
        n: 2,
        text: 'std::vector 是動態陣列，自動管理堆積記憶體，離開作用域時自動釋放，體現 RAII。',
      },
      {
        n: 3,
        text: 'range-based for 搭配 const auto& 走訪容器，避免不必要的複製，也不需手動處理迭代器。',
      },
...
// If your compiler doesn't yet support C++23's <print>, fall back to iostream.
int main() {
