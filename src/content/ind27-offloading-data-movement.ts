...
void OverlappedPipeline(std::size_t totalElems) {
    ...
    // hipHostMalloc(&h_pinned, totalElems * sizeof(float));      // [2] pinned host 記憶體
    float* d_buf = nullptr;
    // hipMalloc(&d_buf, totalElems * sizeof(float));              // device 記憶體

    // hipStream_t streams[kChunks];
    // for (int i = 0; i < kChunks; ++i) {
    //     hipStreamCreate(&streams[i]);                           // [3] 每個區塊一條 stream
    // }

    for (int i = 0; i < kChunks; ++i) {
        std::size_t offset = static_cast<std::size_t>(i) * chunkElems;
        // hipMemcpyAsync(d_buf + offset, h_pinned + offset, chunkBytes,
        //                 hipMemcpyHostToDevice, streams[i]);     // [4] 非同步 H2D，區塊 i
        // hipLaunchKernelGGL(scaleKernel,
        //                     dim3((chunkElems + 255) / 256), dim3(256),
        //                     0, streams[i],
        //                     d_buf + offset, 2.0f,
        //                     static_cast<int>(chunkElems));      // [5] 區塊 i 的計算與區塊 i+1
        //                     的傳輸重疊
    ...
    // for (int i = 0; i < kChunks; ++i) {
    //     hipStreamSynchronize(streams[i]);                       // [6] 同步點：等所有區塊完成
    ...
    callouts: [
      { n: 1, text: '真正的 HIP/CUDA 程式需以 hipcc/nvcc 編譯；此處以註解示意多 stream 重疊的資料流與呼叫形狀。' },
      { n: 2, text: 'hipHostMalloc 配置 pinned（page-locked）記憶體，是達成「真正非同步」DMA 傳輸的前提；一般 malloc 的可分頁記憶體會讓 async 複製悄悄退化為同步。' },
      { n: 3, text: '每個資料區塊配一條獨立 stream，讓不同區塊的傳輸與計算可在硬體上並行排程。' },
      { n: 4, text: 'hipMemcpyAsync 把 H2D 傳輸排入對應 stream 後立即返回，不阻塞 host。' },
      { n: 5, text: '核心啟動同樣非同步：因為排在同一 stream，會等該 stream 的複製完成才執行；不同 stream 之間則可並行，形成傳輸與計算的重疊（pipelining）。' },
      { n: 6, text: 'hipStreamSynchronize 是明確的同步點，用來確保所有區塊都完成後才讀取結果；同步點應盡量集中在最後，而非每個區塊後都呼叫。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: '因為 host/device 交替存取同一塊記憶體時會觸發 page fault 與 page migration，延遲不會顯示在原始碼裡，只在剖析工具的底層計數器才看得到' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: 'DMA 引擎要求傳輸期間來源位址固定不動；可分頁記憶體隨時可能被作業系統搬動或換頁，因此驅動需先同步複製到暫存 pinned buffer，讓 async 複製退化為同步' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '用 rocprof/rocm-smi（AMD）或 Nsight Systems（NVIDIA）量測實際 PCIe/Infinity Fabric 傳輸位元組數與頻寬利用率，與理論頻寬上限比較' },
  ...
  furtherReading: [
    {
      ...
      description: 'HIP 的 host/device 模型、記憶體管理、stream 與非同步 API 的官方說明，本章 API 主要依此撰寫。',
    },
    {
    ...
    },
    {
      ...
      description: '對照用：量測 H2D/D2H 頻寬利用率、stream 時間軸與 kernel launch 開銷的 NVIDIA 剖析工具。',
    },
  ],
};
