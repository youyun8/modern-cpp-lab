...
// 5 點 stencil：out[i] = (in[i-1] + in[i] + in[i+1]) / 3，SoA 佈局（連續陣列）。
void stencilSimd(const std::vector<float>& in, std::vector<float>& out) {
    ...
    for (; i + width < n; i += width) {
        Vec left, mid, right;
        left.copy_from(&in[i - 1], stdx::element_aligned);  // [3] 未強制對齊，安全但可能較慢
...
int main() {
    ...
    callouts: [
      { n: 1, text: 'native_simd<float>::size() 是編譯目標硬體的原生向量寬度（例如 AVX2 float 為 8），不是工程師手動指定的魔術數字。' },
      { n: 2, text: '主迴圈條件 i + width < n 確保每次讀寫都完整落在陣列範圍內，避免越界的向量 load/store。' },
      { n: 3, text: 'element_aligned 表示不假設記憶體對齊到向量寬度；若能保證對齊（如用 aligned_alloc），可改用 vector_aligned 換取更快的 load/store 指令。' },
      { n: 4, text: '(left + mid + right) / Vec(3.0f) 是 lane-wise 運算：width 個元素的加法與除法一次完成，直接映射到硬體向量指令。' },
      { n: 5, text: '尾端 remainder 迴圈是必要的：資料量不是向量寬度整數倍時，剩餘元素只能用純量邏輯逐一補齊，遺漏這段會讀寫越界或漏算。' },
      { n: 6, text: '刻意選一個非向量寬度倍數的 n，是為了在範例裡實際觸發 remainder 路徑，驗證它有被正確覆蓋到。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: 'lane-wise（垂直）運算：第 i 個 lane 的結果只由 a、b 各自的第 i 個 lane 決定' },
        { id: 'c', text: '編譯器會自動決定要不要向量化這個加法' },
        { id: 'd', text: '需要先呼叫 reduce 才能執行加法' },
      ],
      correctOptionId: 'b',
      explanation: 'std::simd 的算術運算子是垂直（lane-wise）的：每個 lane 獨立運算，直接對應硬體向量指令，不涉及跨 lane 的歸約。若要把所有 lane 加總成一個純量，需要另外呼叫歸約函式（如 reduce）。',
    },
    {
      id: 'q2',
      ...
      options: [
        ...
        { id: 'b', text: '因為同一欄位在 AoS 裡不連續，必須用 gather 而非連續 load，吞吐與延遲通常都比純量迴圈更差' },
        { id: 'c', text: '因為 AoS 無法儲存 float 型別' },
        { id: 'd', text: '因為 AoS 會觸發編譯器錯誤，std::simd 無法編譯' },
      ],
      correctOptionId: 'b',
      explanation: 'AoS 佈局下同一欄位（如 x）分散在每個結構體之間相隔固定 stride 的位置，無法連續 load 進 SIMD 暫存器，只能靠 gather 逐一讀取；gather/scatter 的成本常常抵銷甚至超過向量化帶來的效益，因此 SoA 幾乎是有效向量化的前提。',
    },
    {
      id: 'q3',
      ...
      options: [
        ...
        { id: 'b', text: '資料量通常不是向量寬度的整數倍，剩餘不足一個向量寬度的元素只能用純量邏輯逐一處理' },
        { id: 'c', text: 'std::simd 型別不支援讀取陣列中間的元素' },
        { id: 'd', text: '純量迴圈是為了觸發自動向量化的備援路徑' },
      ],
      correctOptionId: 'b',
      explanation: '向量寬度是固定的硬體常數（如 8 或 16），而輸入資料大小是任意的，兩者相除通常有餘數；主迴圈只能處理整數個向量寬度的資料，剩下的尾端元素必須靠額外的純量迴圈補齊，否則會漏算或越界。',
    },
  ],
  diagram: {
    key: 'execution-policies',
    caption: 'std::simd 把一條純量迴圈攤平成固定寬度的 lane：SoA 佈局讓每個 lane 對應到連續記憶體的一次向量 load/store，AoS 佈局則被迫退化成逐 lane 的 gather。',
  },
  tryIt: {
  ...
  furtherReading: [
    {
      ...
      description: '把 std::experimental::simd 扶正為標準 std::simd 的 WG21 提案，涵蓋型別模型、mask 與 ABI 考量。',
    },
    {
