...
int main() {
    ...
    callouts: [
      { n: 1, text: 'std::dextents<size_t, 2> 表示兩個維度皆為執行期動態長度；mdspan 預設 layout 為 layout_right（列優先，與 C 陣列一致）。' },
      { n: 2, text: '函式簽名以 mdspan 攜帶維度資訊，呼叫端不再需要另外傳 n、m、k 或手算 stride。' },
      { n: 3, text: 'a(i, k) 由 mapping（layout policy）轉成線性偏移，再交給 accessor 讀取，取代手寫的 a[i * lda + k]。' },
      { n: 4, text: 'i-k-j 迴圈順序讓內層對 b(k, j) 與 c(i, j) 皆為連續記憶體存取，對 layout_right 的資料快取友善。' },
      { n: 5, text: '一維 mdspan 讓 stencil 的邊界條件（i 從 1 到 n-2）清楚可見，不必記憶原始指標的偏移規則。' },
      { n: 6, text: '權重固定的三點平均；生產程式碼常再以 std::linalg 或向量化版本取代這種純量迴圈。' },
    ],
  },
  deepDive: [
    {
      ...
      body:
        '`std::span`（Ch.19）只能表達一維、連續的「指標 + 長度」；一旦資料邏輯上是矩陣或張量，過去只能靠手算 `i * lda + j` 這類線性化索引，容易寫錯 stride 或搞混列/行主序。`std::mdspan` 把這個問題結構化：它由三個部分組成——`data_handle`（指向底層記憶體，通常是原始指標）、`mapping`（依 layout policy 把多維索引轉成線性偏移）、`accessor`（決定如何真正讀寫記憶體）。`mdspan` 本身仍是非擁有視圖，語意上是 span 的多維推廣，而非容器。',
    },
    {
      heading: 'layout policy：row-major、col-major、strided、tiled',
      body:
        '`layout_right` 是預設佈局，最後一個維度變化最快，對應 C／C++ 陣列的列優先（row-major）慣例。`layout_left` 則是行優先（column-major），對應 Fortran 與傳統 BLAS／LAPACK 的記憶體序，用來直接包裝呼叫 Fortran BLAS 的緩衝區而不必轉置。`layout_stride` 允許任意步幅，可表達子矩陣視圖（例如從大矩陣中切出一塊而不複製）或對齊填充（padding）後的列。\n\n更進階的自訂 layout（例如切成小塊、對 cache line 對齊的 tiled layout）可以把資料按快取區塊重新排列，讓 GEMM 這類演算法在存取時天然具有區域性；標準未內建 tiled layout，但 mapping policy 是一個可自訂的 customization point，函式庫或使用者可自行實作符合 `LayoutMapping` 概念的型別。',
    },
    {
      heading: 'accessor policy：存取的自訂點',
      body:
        '`accessor_policy` 決定「拿到線性偏移之後，如何真正讀寫該筆資料」，預設的 `default_accessor<T>` 就是普通的指標解參考。這個層次可以插入額外語意：例如把讀寫包成 `std::atomic_ref` 以支援多執行緒安全存取、在讀取時做型別轉換或量化解碼（把底層 `int8_t` 解讀成縮放後的 `float`），或加入邊界檢查版本供除錯建置使用。\n\nlayout 與 accessor 是正交的兩個 customization point：同一份 layout 可以搭配不同 accessor，反之亦然，這讓 mdspan 能同時服務「純索引運算」與「特殊記憶體語意」兩種需求，而不必為每種組合寫專門的容器型別。',
    },
    {
      heading: 'std::linalg（P1673）與退回廠商 BLAS 的時機',
      body:
        '`std::linalg`（P1673，鎖定 C++26，部分編譯器／函式庫已提供早期實作）在 mdspan 之上定義一組 BLAS 風格演算法：`dot`、`scale`、`matrix_product`（GEMM）、三角求解等，介面以 mdspan 表達運算元，語意可攜、型別安全，且不需要外部函式庫即可編譯連結。它適合作為「隨標準庫附帶的合理預設」，或用在可攜性優先於極致效能的場合。\n\n但 std::linalg 的參考實作通常不會達到硬體峰值：真正的效能來自針對特定 ISA、快取階層與 GPU 架構調校多年的廠商函式庫——AMD GPU 上的 rocBLAS、NVIDIA GPU 上的 cuBLAS、x86 CPU 上的 Intel MKL（或 OpenBLAS）。工業實務上常見的模式是：介面以 mdspan／std::linalg 的形式撰寫，讓程式碼可攜且可讀；效能關鍵的呼叫點則在建置時依目標硬體 dispatch 到對應的廠商 BLAS，必要時把 mdspan 的 data handle 直接傳給廠商 API（它們大多也接受裸指標 + leading dimension，與 mdspan 的 mapping 資訊一一對應）。',
    },
    {
      heading: '重寫 GEMM／stencil：索引清晰度而非新演算法',
      body:
        '把本書貫穿全書的 GEMM 範例（Ch.26 軟體設計章節、Ch.19 span）改寫成 mdspan 版本，重點不在改變演算法本身（i-k-j 迴圈順序、分塊、向量化等最佳化仍然適用），而是讓函式簽名攜帶維度與佈局資訊：`gemm(mdspan<const float, dextents<size_t,2>> a, ...)` 取代 `gemm(int n, const float* a, ...)`，呼叫端不再需要另外傳長度與 stride，也不會把列優先與行優先搞混。同樣地，stencil 的邊界條件（例如 1D 三點平均只在 `[1, n-2]` 上有效）用 `extent(0)` 表達比裸指標加迴圈更不容易寫錯。這種「索引清晰度」的收益在多維、多層迴圈的數值核心中尤其明顯。',
    },
  ],
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: 'mdspan 是把 span 的非擁有連續視圖概念推廣到多維，兩者都不擁有底層記憶體' },
        { id: 'c', text: 'mdspan 只能用於 GPU 記憶體，span 只能用於 CPU 記憶體' },
        { id: 'd', text: 'mdspan 是 span 的子類別，繼承其邊界檢查行為' },
      ],
      correctOptionId: 'b',
      explanation: 'mdspan 與 span 一樣是非擁有視圖，差別在於 mdspan 透過 layout policy 支援多維索引；兩者的生命週期都依附於底層緩衝區。',
    },
    {
      id: 'q2',
      ...
      options: [
        ...
        { id: 'b', text: '因為廠商 BLAS 針對特定硬體的快取階層、SIMD／張量核心與多執行緒調校多年，效能通常遠優於可攜的參考實作' },
        { id: 'c', text: '因為 mdspan 無法傳遞給廠商 BLAS 的 API' },
        { id: 'd', text: '因為廠商 BLAS 不需要正確的 layout 資訊' },
      ],
      correctOptionId: 'b',
      explanation: 'std::linalg 提供可攜且型別安全的預設實作，但真正逼近硬體峰值仍需依賴針對目標架構深度調校的廠商 BLAS；mdspan 的 data handle 與 stride 資訊通常可直接對應到這些 API 的參數。',
    },
    {
      id: 'q3',
      ...
      correctOptionId: 'b',
      explanation: 'layout_left 對應行優先（column-major）記憶體序；若誤用預設的 layout_right 檢視行優先資料，索引到偏移的映射會不正確，導致錯誤結果或跨步幅的低效存取。',
    },
  ],
  diagram: {
    key: 'generic-flow',
