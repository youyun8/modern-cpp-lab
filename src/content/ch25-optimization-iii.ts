...
perf record -g ./app && perf report                    # [5]`,
    callouts: [
      {
        n: 1,
        text: '-O3 積極最佳化、-march=native 啟用當前 CPU 指令集（含 SIMD），-g 保留符號以便剖析。',
      },
      { n: 2, text: '-flto 讓最佳化跨越轉譯單元邊界，可內聯原本無法內聯的跨檔函式並移除死碼。' },
      { n: 3, text: '-fprofile-generate 產生帶插樁的版本，執行後蒐集實際分支與熱路徑資料。' },
      {
        n: 4,
        text: '-fprofile-use 以剛才的剖析回饋指導最佳化（分支機率、版面配置），常帶來額外增益。',
      },
  ...
  deepDive: [
    {
      heading: '最佳化等級與語意的取捨',
      body: '`-O2` 是穩健預設；`-O3` 更積極（更多內聯與向量化）但偶爾使程式變大甚至變慢；`-Os` 以體積為目標。`-march=native` 啟用當前 CPU 指令集但犧牲可攜性，不適合散布給未知硬體。\n\n`-ffast-math` 會改變浮點語意（假設無 NaN／inf、允許重排），可能破壞正確性。LTO 讓跨單元內聯與死碼移除，通常值得開啟。',
    },
    {
      heading: '剖析方法：取樣與插樁',
      body: '取樣式剖析（perf、VTune）以低負擔定期擷取呼叫堆疊，適合找熱點；插樁式（callgrind、gprof）提供精確呼叫計數但負擔高。火焰圖（flame graph）能直觀呈現時間分佈。\n\n`perf stat` 可看快取未命中、分支預測失敗與 IPC 等硬體計數器，判斷瓶頸性質。務必對 Release 建置剖析，並確保有除錯符號以對應原始碼。',
    },
    {
      heading: '嚴謹的微基準',
      body: '微基準需暖身（warmup）、足夠迭代與統計（中位數／變異數），並以 `benchmark::DoNotOptimize` 防止結果被死碼消除。CPU 頻率調節（turbo）與其他程序會製造雜訊，應固定頻率、綁定 CPU 以獲得穩定數字。\n\n注意微基準的結果未必反映真實工作負載下的快取與分支行為；最終仍應以端到端量測驗證。',
    },
  ],
  pitfalls: [
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '可使用當前 CPU 的指令集（如 AVX）提升效能，但產物可能無法在較舊 CPU 上執行',
        },
        { id: 'c', text: '它一定會讓程式變慢' },
