...
const ch16CodeConventionsII: ChapterContent = {
  ...
  concept: {
    standard: 'C++23',
    body:
      '現代 C++ 提供許多讓程式更安全、更易讀的慣用寫法。auto 減少冗長型別、避免隱式窄化與型別不符，但在會傷害可讀性處仍應寫明型別。結構化繫結（auto [k, v] = ...）讓解構 pair、tuple 與結構更自然。range-based for 取代易錯的索引迴圈，C++20 ranges 進一步支援 views::filter、views::transform 等惰性、可組合的管線。一致初始化以大括號 {} 統一語法並防止窄化轉換。其他要點：優先使用 nullptr 而非 NULL、用 enum class、用 using 別名取代 typedef、以 std::array 取代 C 陣列。核心精神是讓意圖直接寫在程式碼裡。',
  },
  code: {
    lang: 'cpp',
    code: `#include <map>
...
int main() {
  ...
  return 0;
    ...
    callouts: [
      { n: 1, text: '結構化繫結把 map 元素直接解構為 name 與 score，比 it->first／it->second 清楚得多。' },
  ...
  deepDive: [
    {
      ...
      body:
        '類別樣板引數推導（CTAD）讓 `std::vector v{1,2,3}` 免寫型別；結構化繫結解構 pair／tuple／聚合；ranges 以 `|` 串接惰性視圖形成宣告式資料流。\n\n這些特性減少樣板程式碼與出錯面，但要理解其成本模型：視圖是惰性的、不複製，卻也因此對來源生命週期敏感。',
    },
    {
      heading: '現代錯誤處理',
      body:
        '可預期的失敗以 `std::optional`（無值）或 `std::expected`（值或錯誤，C++23）回傳，讓錯誤進入型別系統，取代以特殊回傳值或輸出參數表達失敗。真正的例外情況才用 exceptions。\n\n以 `[[nodiscard]]` 標記回傳值不可忽略的函式（尤其回傳 `expected`／錯誤碼者），讓忽略錯誤成為編譯警告。',
    },
    {
      heading: '可讀性、零成本與生命週期',
      body:
        'ranges 與 `auto` 提升可讀性且多為零成本，但 `std::string_view`／`std::span` 與視圖都是非擁有的：指向暫時物件會懸置（如 `std::string_view sv = get_string();` 其中回傳暫時 `std::string`）。\n\n把視圖用作函式參數（唯讀、不儲存）是安全常見的用法；把視圖存進成員或回傳指向暫時物件的視圖則危險。',
    },
  ],
  pitfalls: [
...
int main() {
  ...
  int total = 0;
  for (int x : evens) { std::cout << x << ' '; total += x; }
  std::cout << "\\nsum = " << total << '\\n';
  return 0;
}`,
  },
