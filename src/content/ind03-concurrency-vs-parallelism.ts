...
void bspRelaxationStep(std::vector<double>& grid, int iterations) {
    ...
    callouts: [
      {
        n: 1,
        text: '任務平行：兩段程式碼做「不同的事」（解析設定檔 vs 預熱磁碟快取），彼此無資料相依，可同時進行。',
      },
      {
        n: 2,
        text: 'std::async 啟動獨立任務並回傳 future，是 fork-join 中「fork」的標準函式庫寫法。',
      },
      {
        n: 3,
        text: 'get() 會阻塞直到對應任務完成，兩條分支在此處匯合——這就是 fork-join 的「join」。',
      },
  ...
  quiz: [
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '並行是程式結構上能否交錯處理多個獨立任務，平行是這些任務能否真的同時在多個運算單元上執行',
        },
    ...
    },
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '標記每一輪超步（superstep）結束的可重複使用同步屏障，確保所有執行緒完成本地運算後才進入下一輪',
        },
  ...
  diagram: {
    ...
    caption:
      '三種平行範式（任務／資料／管線）到 HPC 常見拓撲（SPMD、fork-join、BSP）的心智模型演進。',
  },
  tryIt: {
