...
int main() {
    ...
    callouts: [
      { n: 1, text: '這是經典的 Store Buffering（SB）litmus test：兩執行緒各自寫自己的旗標、讀對方的旗標。' },
      { n: 2, text: 'relaxed store 只保證這個變數自身的修改順序，不對其他變數的可見性做任何保證。' },
      { n: 3, text: '獨立的 atomic_thread_fence(seq_cst) 在 store 與 load 之間建立屏障，阻止兩者被重排到彼此之前，並讓所有執行緒對這個順序達成共識。' },
      { n: 4, text: 'thread2 對稱地寫自己的旗標，順序與 thread1 相同但變數互換。' },
      { n: 5, text: '若沒有 fence，弱序硬體允許兩個 store 都停留在各自的 store buffer 中，導致兩個 load 都讀到舊值 0——這在 x86 上罕見，在 ARM 上並不罕見。' },
      { n: 6, text: '加上成對的 seq_cst fence 後排除了 r1==0 && r2==0 這個結果，程式行為在 x86 與 ARM 上一致，代價是屏障帶來的效能開銷。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: 'x86 是強序（TSO）架構，硬體本身限制了重排；ARM 是弱序架構，允許更大幅度的 store/load 重排' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '在 relaxed 順序下，兩個 load 可以同時讀到 0（各自的 store 仍停留在 store buffer 中）' },
        { id: 'c', text: '程式一定會死鎖' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '因為依賴鏈（carries-a-dependency-to）在最佳化過程中極難正確追蹤，所有主流編譯器都直接把它實作成 acquire，得不到理論上的效能優勢' },
...
int main() {
  ...
  furtherReading: [
    ...
    },
    {
      ...
      description: '完整列出 relaxed／consume／acquire／release／acq_rel／seq_cst 的正式定義，含 consume 目前實作狀態的說明。',
    },
    {
      title: 'CppMem: Interactive C/C++ Memory Model',
      href: 'https://www.cl.cam.ac.uk/~pes20/cppmem/',
      description: '劍橋大學開發的線上工具，可窮舉列出 litmus test 在 C++ 記憶體模型下所有允許的執行結果。',
    },
    {
