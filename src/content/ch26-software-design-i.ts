...
void run(const Kernel& k, int n, const std::vector<float>& A, const std::vector<float>& B,
    ...
    callouts: [
      ...
      {
        n: 5,
        text: '開放封閉（OCP）：要加入分塊或 SIMD 版本，只需新增 Kernel 子型別，無須改動 run。',
      },
    ],
  },
  deepDive: [
    {
      heading: '抽象的成本與擺放位置',
      body: '抽象應放在變化頻繁、非效能關鍵的邊界，而非緊迴圈內。執行期虛擬多型有間接呼叫成本；效能關鍵處可改用靜態多型（樣板／CRTP）或 policy-based design，在編譯期解析並保留內聯。\n\n關鍵原則：在外層保留彈性的抽象邊界，在內層熱路徑追求具體與零成本。過早或過度抽象會同時傷害效能與可讀性。',
    },
    {
      heading: 'GEMM 的最佳化階梯',
      body: '天真的三重迴圈 GEMM 遠低於硬體峰值。專業 BLAS 透過分塊／tiling（切成放得進快取的區塊）、資料打包（packing 成連續且對齊）、暫存器分塊、向量化與多執行緒逐層逼近峰值。\n\n這說明「相同的漸進複雜度，實作差異可達數十倍」。工業上除非有特殊需求，應直接採用廠商 BLAS（如 OpenBLAS、MKL、hipBLASLt），而非自行重寫。',
    },
    {
      heading: '值語意與 API 設計',
      body: '值語意（複製、無別名、易推理）應為預設，讓函式易於測試與並行；需要共享或多型時才引入參考／指標並釐清所有權。函式介面以 `std::span`／`std::string_view` 接受唯讀序列，避免不必要的樣板化與複製。\n\n清楚的所有權與最小化的可變共享狀態，是既正確又高效設計的基礎。',
    },
  ],
  pitfalls: [
