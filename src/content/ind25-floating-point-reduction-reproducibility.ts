...
int main() {
    ...
    callouts: [
      ...
      { n: 3, text: '當目前部分和量級較大時，新元素 x 的低位元在對齊指數時被捨去，故從 x 這一側回推遺失量。' },
      { n: 4, text: 'Neumaier 相對原始 Kahan 的改進：新元素比部分和大時改用另一側計算，避免原始 Kahan 在此情況失效。' },
      { n: 5, text: '回傳前把累積的補償 c 加回 sum，修正前面所有步驟遺失的低位元。' },
      { n: 6, text: 'Pairwise 求和把陣列遞迴對半切，形成平衡二元樹；樹形狀固定則與執行緒數切分無關，具確定性。' },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: 'c 記錄每一步加法因捨入而理論上遺失的低位元量，下次加法前補回，藉此抑制誤差累積' },
    ...
    },
    {
      ...
      options: [
        ...
        { id: 'b', text: '用高精度參考實作算出真值，再依元素個數、資料範數與條件數推導相對誤差容差進行比較' },
