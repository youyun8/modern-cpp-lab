...
// instead of merely running slower. [6]`,
    callouts: [
      ...
      {
        n: 5,
        text: '在 par_unseq 下取得鎖是明確禁止的行為：交錯執行可能讓同一執行緒重入未釋放的鎖。',
      },
      {
        n: 6,
        text: '配置記憶體或讓例外逃逸同樣違反向量化安全契約，結果是未定義行為或 std::terminate，而非單純變慢。',
      },
    ],
  },
  pitfalls: [
  ...
  quiz: [
    {
      ...
      options: [
        ...
        {
          id: 'b',
          text: '因為向量化交錯可能讓同一執行緒在釋放鎖前重入，造成死鎖，且標準明訂此為未定義行為',
        },
      ...
      correctOptionId: 'b',
      explanation:
        'par_unseq 允許把多次元素呼叫在同一執行緒內交錯執行，若其中一次呼叫尚未釋放鎖就被交錯進另一次呼叫，會直接死鎖；標準將此情境定義為未定義行為，而非僅是效能問題。',
    },
    {
      id: 'q2',
      ...
      correctOptionId: 'b',
      explanation:
        '截至 C++23，多數 std::ranges 演算法仍是純循序介面，尚未整合執行策略；相關整合方向仍有委員會提案在演進中，實務上要平行化通常仍需退回迭代器版本的演算法。',
    },
    {
      id: 'q3',
      ...
      correctOptionId: 'b',
      explanation:
        'nvc++ 的 stdpar GPU offload 要求所有被平行演算法觸及的記憶體都能被 GPU 存取，實務上依賴 CUDA 統一／受管理記憶體，且只涵蓋部分演算法與呼叫模式；記憶體不相容時會退回主機執行或直接失敗。',
    },
  ],
  diagram: {
    key: 'execution-policies',
    caption:
      '切換分頁比較 seq／par／unseq／par_unseq 如何把 8 個元素分派到執行緒與向量通道——注意 unseq／par_unseq 允許的交錯正是禁止上鎖與例外逃逸的根本原因。',
  },
  tryIt: {
    code: `#include <algorithm>
#include <execution>
...
int main() {
  ...
  furtherReading: [
    ...
    },
    {
      ...
      description:
        'AMD ROCm 工具鏈文件，涵蓋 roc-stdpar 一類標準平行化 GPU offload 效果的現況與限制。',
    },
  ],
};
