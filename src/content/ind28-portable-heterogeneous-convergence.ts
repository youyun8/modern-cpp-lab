...
//   stdexec::sync_wait(work);`,
    callouts: [
      { n: 1, text: 'SYCL 版本會用 parallel_for 搭配 USM 指標表達完全相同的逐元素加法，語法不同但結構一致。' },
      { n: 2, text: 'HIP 版本會啟動一個明確撰寫的 kernel，語法幾乎與對應的 CUDA kernel 一一對應。' },
      { n: 3, text: 'CUDA 版本結構相同，只是換成 <<<>>> 語法與 __global__ kernel 函式。' },
      { n: 4, text: '這是「隱藏派送」路線：呼叫端完全不改寫演算法邏輯，是否 offload 到 GPU 由編譯器與工具鏈決定。' },
      { n: 5, text: 'par_unseq 只是「請求」平行與向量化派送，實際能否落地到 GPU 仍取決於記憶體來源與編譯旗標。' },
  ...
  quiz: [
    {
      ...
      options: [
        ...
        { id: 'b', text: '語法幾乎與 CUDA Runtime API 一一對應，並提供 hipify 工具半自動轉換既有 CUDA 程式碼，同時可編譯到 AMD 與 NVIDIA 硬體' },
        { id: 'c', text: '它是 Khronos 制定的開放標準，因此比 CUDA 更快' },
        { id: 'd', text: '它不需要任何程式碼修改就能自動比 CUDA 快' },
      ],
      correctOptionId: 'b',
      explanation: 'HIP 是 AMD 主推、語法貼近 CUDA 的可攜 API，搭配 hipify 工具可從 CUDA 原始碼半自動轉換，同一份 HIP 程式碼可編譯到 AMD 原生 ROCm 或轉呼叫底層 CUDA，藉此降低熟悉 CUDA 的團隊遷移到 AMD 硬體的成本。',
    },
    {
      id: 'q2',
      ...
      options: [
        ...
        { id: 'b', text: 'std::par 讓編譯器隱式決定是否 offload，使用者不改寫演算法呼叫；std::execution 則要求顯式選擇 scheduler，把派送機制做成第一級抽象' },
        { id: 'c', text: 'std::execution 只能在 CPU 上執行，std::par 只能在 GPU 上執行' },
        { id: 'd', text: '兩者都需要完全重寫成 CUDA 語法才能運作' },
      ],
      correctOptionId: 'b',
      explanation: 'std::par 路線的價值在於幾乎不需改寫既有標準演算法呼叫，是否落地到 GPU 由編譯器與工具鏈隱式決定；std::execution 路線則要求使用者顯式選擇 scheduler 並用 bulk 等演算法描述資料平行工作，兩者是「隱藏派送」與「顯式但泛用的派送抽象」這同一收斂目標的兩種表現形式。',
    },
    {
      id: 'q3',
      ...
      options: [
        ...
        { id: 'b', text: '因為廠商函式庫針對特定硬體世代的暫存器、共享記憶體、tensor/matrix core 資料佈局做了泛用編譯器難以自動重現的極致調校' },
        { id: 'c', text: '因為廠商函式庫是唯一能在 GPU 上執行的程式碼' },
        { id: 'd', text: '因為標準委員會明文禁止把 std::linalg offload 到 GPU' },
      ],
      correctOptionId: 'b',
      explanation: '廠商函式庫投入大量工程針對每一代硬體的暫存器檔案、共享記憶體 bank、tensor/matrix core 資料佈局與 tile 策略做手工調校，這種程度的特化目前泛用編譯器難以從標準演算法呼叫自動推導出來，因此在最效能敏感的內層迴圈，手調 kernel 或廠商函式庫通常仍勝過標準 C++ 路線。',
    },
  ],
  diagram: {
    key: 'generic-flow',
    nodes: ['SYCL', 'HIP', 'CUDA', 'std::execution'],
    caption: '四種模型收斂於同一目標——以標準或近標準 C++ 描述平行計算，再把「派送到哪個硬體」交給編譯器、hipify 轉換或 scheduler 抽象決定。',
  },
  tryIt: {
    code: `#include <algorithm>
#include <execution>
  ...
  furtherReading: [
    {
      ...
      description: 'SYCL 官方規格文件，涵蓋單一原始碼模型、buffer/accessor 與 USM 記憶體模型的完整定義。',
    },
    {
      ...
      description: 'AMD 官方的 CUDA 到 HIP 移植指南，涵蓋 hipify 工具用法與需要人工審查的常見情境。',
    },
    {
    ...
    },
    {
      ...
      description: 'senders/receivers 與 std::execution 的正式提案文件，是 scheduler/bulk 抽象的標準化依據。',
    },
  ],
};
